...
import yusuke.client.module.setting.impl.*;
import yusuke.client.util.ClickUtil;
import yusuke.client.util.ColorUtil;
import yusuke.client.util.animation.AnimationUtil;
import yusuke.client.util.animation.LinearAnimation;
import yusuke.font.FontManager;
import yusuke.font.FontRenderer;
...
public class ClickGuiWindow {
    ...
    private FontRenderer font = FontManager.get(20);
    private FontRenderer mini = FontManager.get(18);
    private FontRenderer setting = FontManager.get(15);

    private ModuleSetting focusedSetting = null;

    private void rect(double x, double y, double width, double height, int color) {
    ...
    public ClickGuiWindow(float x, float y, ModuleCategory category) {
        ...
        modules = Yusuke.getModuleManager()
                .getModules()
                .stream()
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        font.draw(category.getName(), x + 25, y + 8, 0xffE0DFE2);
        float offset = y + 24;
        for (int i = 0; i < modules.size(); i++) {
            Module m = modules.get(i);
            rect(x, offset, 125, 18, 0xff1D1D21);
            m.toggleAnimation.uodate(m.isToggled() ? 0.05 : -0.05);
            mini.drawCentered(m.getName(), x + 62.5f, offset + 5, ColorUtil.interpolateColor(0xffE0DFE2, getColor(), (float) m.toggleAnimation.calcPercent()));
            offset += 18;
            if (!mExpand[i]) continue;
            for (ModuleSetting s : m.getSettingList()) {
                if (s instanceof BooleanSetting) {
                    rect(x, offset, 125, 18, 0xff151519);
                    setting.draw(s.getName(), x + 5, offset + 7, 0xffE0DFE2);
                    ...
                    rect(x, offset, 125, 18, 0xff151519);
                    setting.draw(s.getName(), x + 5, offset + 5, 0xffE0DFE2);
                    ...
                    setting.drawRight(String.valueOf(ds.getValue()), x + 120, offset + 5, 0xffE0DFE2);
                    offset += 18;
                    ...
                    setting.drawCentered(s.getName(), x + 62.5f, offset + 7, 0xffE0DFE2);
                    offset += 18;
...
//                    StencilUtil.readStencilBuffer();
                    if (ms.expand) {
                        for (int mi = 0; mi < ms.getOption().length; mi++) {
                            ...
                            setting.drawCentered(ms.getOption()[mi], x + 62.5f, cpOffset + 3, mi == ms.getIndex() ? getColor() : 0xffE0DFE2);
                            cpOffset += 14;
                        }
                    }
...
//                    StencilUtil.readStencilBuffer();
                    for (Map.Entry<String, Boolean> entry : ms.getValue().entrySet()) {
                        rect(x, cpOffset, 125, 18, 0xff151519);
                        rect(x + 7, cpOffset - 2, 111, 14, 0xff202226);
                        setting.drawCentered(entry.getKey(), x + 62.5f, cpOffset + 3, entry.getValue() ? getColor() : 0xffE0DFE2);
                        cpOffset += 14;
                    }
