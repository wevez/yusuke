...
public final class Type
{
    ...
    public static Type[] getArgumentTypes(final String methodDescriptor) {
        ...
        while (methodDescriptor.charAt(currentOffset) != ')') {
            ...
            if (methodDescriptor.charAt(currentOffset++) == 'L') {
                final int semiColumnOffset = methodDescriptor.indexOf(59, currentOffset);
        ...
        while (methodDescriptor.charAt(currentOffset) != ')') {
            ...
            if (methodDescriptor.charAt(currentOffset++) == 'L') {
                final int semiColumnOffset2 = methodDescriptor.indexOf(59, currentOffset);
                currentOffset = Math.max(currentOffset, semiColumnOffset2 + 1);
            }
    ...
    static int getReturnTypeOffset(final String methodDescriptor) {
        ...
        for (currentOffset = 1; methodDescriptor.charAt(currentOffset) != ')'; currentOffset = Math.max(currentOffset, semiColumnOffset + 1)) {
            while (methodDescriptor.charAt(currentOffset) == '[') {
                ++currentOffset;
            }
            if (methodDescriptor.charAt(currentOffset++) == 'L') {
