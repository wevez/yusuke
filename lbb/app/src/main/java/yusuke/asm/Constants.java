...
final class Constants
{
    ...
    static void checkAsmExperimental(final Object caller) {
        ...
        final String internalName = callerClass.getName().replace('.', '/');
    ...
    static boolean isWhitelisted(final String internalName) {
        if (!internalName.startsWith("yusuke/asm/")) {
            return false;
        }
        final String member = "(Annotation|Class|Field|Method|Module|RecordComponent|Signature)";
    ...
    static void checkIsPreview(final InputStream classInputStream) {
        ...
        int minorVersion;
        try {
            final DataInputStream callerClassStream = new DataInputStream(classInputStream);
            try {
